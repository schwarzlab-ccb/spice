...
def _observed_table_override(stem):
    ...
    rather than sharing one build-wide default. Absent -> the packaged per-assembly file, which is the
...
def load_centromeres(extended=True, observed=False, pad=None):
    ...
    override = _observed_table_override(stem) if observed else None
    if override is not None:
        log_debug(logger, f'Loading {stem} from the cohort override {override}')
        centromeres = pd.read_csv(override, sep='\t', header=[0, 1], index_col=0)
    else:
...
def load_telomeres_observed():
    '''Create file using create_observed_centromeres_and_telomeres'''
    override = _observed_table_override('telomeres_observed')
    if override is not None:
        log_debug(logger, f'Loading telomeres_observed from the cohort override {override}')
        telomeres_observed = pd.read_csv(override, sep='\t', header=[0, 1], index_col=0)
    else:
